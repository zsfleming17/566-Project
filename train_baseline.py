# train_baseline.py - trains MobileNetV3-Small for obstructed seat classification
...
def make_model():
    ...
    m.classifier[3] = nn.Linear(in_f, 2) # replace final layer for binary classification
    return m
...
def train(kind="random"):
    ...
    dl_tr, dl_va, dl_te, tr_csv = make_loaders(kind=kind)
    m = make_model().to(device)
    crit = nn.CrossEntropyLoss(weight=class_weights(tr_csv).to(device))
